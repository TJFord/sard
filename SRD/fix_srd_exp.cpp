...
{
//   fix id group-id srd N groupbig-id tsrd hgrid seed (keyword value)->optional
  ...
  nevery = atoi(arg[3]); //atoi: str->int id=0, group-id=1,,, N=3

  bigexist = 1;
  if (strcmp(arg[4],"NULL") == 0) bigexist = 0; // no big particles
  else biggroup = group->find(arg[4]); //biggroup: int id
//  LAMMPS_NS class Group *group;            // groups of atoms
//   int bigexist,biggroup,biggroupbit; // defined in fix_srd.h

  temperature_srd = atof(arg[5]); // tsrd
  gridsrd = atof(arg[6]); //hgrid
  int seed = atoi(arg[7]); //seed
  ...
  me = comm->me; //int me,nprocs; in .h file
  nprocs = comm->nprocs;

  random = new RanMars(lmp,seed + me); //  class RanMars *random; in .h file

  // if requested, initialize shift RNG, same on every proc

  if (shiftuser == SHIFT_YES || shiftuser == SHIFT_POSSIBLE)//  int shiftuser .h
    randomshift = new RanPark(lmp,shiftseed); //class RanPark *randomshift; .h
  ...
  nmax = 0;  //max of srd particle
  binhead = NULL; // 1st SRD particle in each bin
  maxbin1 = 0;
  binnext = NULL; // next SRD particle in same bin
  maxbuf = 0;
  sbuf1 = sbuf2 = rbuf1 = rbuf2 = NULL; // buffers for send/recv of velocity bin data
